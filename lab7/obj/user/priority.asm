...
000000000080015e <lab6_set_priority>:
}
...
main(void) {
  ...
  8005c2:	b9dff0ef          	jal	ra,80015e <lab6_set_priority>
  ...
  8006b4:	6989                	lui	s3,0x2
               lab6_set_priority(i + 1);
  8006b6:	aa9ff0ef          	jal	ra,80015e <lab6_set_priority>
                    if(acc[i]%4000==0) {
  8006ba:	fa04849b          	addiw	s1,s1,-96
               acc[i] = 0;
  8006be:	00042023          	sw	zero,0(s0)
                        if((time=gettime_msec())>MAX_TIME) {
  8006c2:	71098993          	addi	s3,s3,1808 # 2710 <_start-0x7fd910>
  ...
  8006ea:	c014                	sw	a3,0(s0)
                        if((time=gettime_msec())>MAX_TIME) {
  8006ec:	a71ff0ef          	jal	ra,80015c <gettime_msec>
  8006f0:	0005091b          	sext.w	s2,a0
  8006f4:	fd29d9e3          	bge	s3,s2,8006c6 <main+0x13a>
                            cprintf("child pid %d, acc %d, time %d\n",getpid(),acc[i],time);
  8006f8:	a63ff0ef          	jal	ra,80015a <getpid>
  ...
  80070a:	999ff0ef          	jal	ra,8000a2 <cprintf>
                            exit(acc[i]);
  80070e:	4008                	lw	a0,0(s0)
  800710:	a2fff0ef          	jal	ra,80013e <exit>
  800714:	00001417          	auipc	s0,0x1
  800718:	91840413          	addi	s0,s0,-1768 # 80102c <pids+0x14>
          if (pids[i] > 0) {
  80071c:	00092503          	lw	a0,0(s2)
  800720:	00a05463          	blez	a0,800728 <main+0x19c>
               kill(pids[i]);
  800724:	a35ff0ef          	jal	ra,800158 <kill>
     for (i = 0; i < TOTAL; i ++) {
  ...
  800742:	8e5ff0ef          	jal	ra,800026 <__panic>
